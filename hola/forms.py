...
class PrestamoForm(forms.ModelForm):
    ...
    def clean_libro(self):
        ...
        if libro.ejemplares == 0:
            ...
            f"El libro '{libro.titulo}' no está disponible por el momento."
...
class ReservaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Mostrar solo libros con ejemplares disponibles
        disponibles = [libro.id for libro in Libro.objects.all() if libro.disponible]
        self.fields['libro'].queryset = Libro.objects.filter(id__in=disponibles)
