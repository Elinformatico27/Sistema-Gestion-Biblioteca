...
class Libro(models.Model):
    ...
    disponible = models.BooleanField(default=True)  # ahora es normal, editable
...
class Prestamo(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:
            self.fecha_limite = date.today() + timedelta(days=1)
...
# ============================
# Señal: Enviar correo al crear una multa
# ============================
@receiver(post_save, sender=Multa)
def notificar_multa(sender, instance, created, **kwargs):
    if created and instance.prestamo:
        usuario = instance.prestamo.usuario
        mensaje = f"""
Estimado {usuario.nombre},

Se le ha registrado una multa de RD${instance.monto}
por el préstamo del libro "{instance.prestamo.libro.titulo}".

Por favor, regularice su situación en la biblioteca.
"""
        enviar_notificacion(usuario, "Nueva multa en la biblioteca", mensaje)

# ============================
# Señal: Enviar correo al crear una notificación
# ============================
...
Tiene una nueva notificación en el sistema de biblioteca:

"{instance.mensaje}"
"""
        enviar_notificacion(usuario, "Nueva notificación de la biblioteca", mensaje)
